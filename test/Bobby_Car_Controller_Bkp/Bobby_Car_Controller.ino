/**
 * Probably an old version of the bobby car controller code
 */

#include <SoftwareSerial.h>

// Pin for analog X joystick (Nunchuck)
#define JOY_Y_PIN A0 // Analog pin for X-axis
...
SoftwareSerial hoverboardSerial(RX_PIN,
...
#ifdef DEBUG_MODE
  Serial.begin(
      9600); // Start serial monitor for debugging if debug mode is enabled
...
void loop() {
  int speed =
      readJoystick();      // Read and map the joystick value to the speed range
  sendToHoverboard(speed); // Send the speed value to the hoverboard via USART
  readHoverboardFeedback(); // Read and process feedback from the hoverboard
...
detectDrivingMode
  ...
  // Convert speed into a serial packet (adjust based on motor controller
  // documentation)
...
// Print the feedback for debugging
      ...
      Serial.println(feedback.cmd1);
      Serial.print("Command 2: ");
      Serial.println(feedback.cmd2);
      Serial.print("Speed Left: ");
      Serial.println(feedback.speedLeft);
      Serial.print("Speed Right: ");
      Serial.println(feedback.speedRight);
      Serial.print("Current Left: ");
      Serial.println(feedback.currentLeft);
      Serial.print("Current Right: ");
      Serial.println(feedback.currentRight);
