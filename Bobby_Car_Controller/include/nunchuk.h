...
static void nunchuk_init_power() {
    constexpr uint8_t kPortC2Mask = static_cast<uint8_t>(1u << PORTC2);
    constexpr uint8_t kPortC3Mask = static_cast<uint8_t>(1u << PORTC3);

    // Add power supply for port C2 (GND) and C3 (PWR)
    // cppcheck-suppress cstyleCast
    PORTC &= static_cast<uint8_t>(~kPortC2Mask);
    // cppcheck-suppress cstyleCast
    PORTC |= kPortC3Mask;
    // cppcheck-suppress cstyleCast
    DDRC |= static_cast<uint8_t>(kPortC2Mask | kPortC3Mask);
    delay(100);
}
#endif
/**
