...
SoftwareSerial HoverSerial(RX_PIN,
                           TX_PIN); // RX, TX pins for USART communication
...
/**
 * @brief Runs joystick-based drive mode detection and updates global drive
 * mode.
 *
...
#ifdef DEBUG_MODE
  Serial.begin(DEBUG_SERIAL_BAUD); // Start serial monitor for debugging if
...
void loop() {
  if (driveMode > 0 && driveMode < 5) {
    ...
    // Get latest Nunchuk input (Y axis for throttle/brake, buttons for mode
    // switch).
    ...
    // TODO Make a nunchuk poti release required when going from forward in
    // reverse
    ...
    if ((acc_cmd < ACC_ACTIVE_THRESHOLD && brk_cmd < ACC_ACTIVE_THRESHOLD) || ignoreBrk) {
      ...
      speed = speed * (1.0 - FREEWHEELING_DECELERATION); //(speed > 0 ? MODES[i].ACC_FORWARDS
                                                         /// MODES[i].MAX_SPEED_FORWARDS *
                                                         //FREEWHEELING_DECELERATION * 1.0 :
                                                         //MODES[i].ACC_REVERSE /
                                                         //MODES[i].MAX_SPEED_REVERSE *
                                                         //FREEWHEELING_DECELERATION * 1.0));
...
bool detectDrivingMode() {
  ...
  while (!modeFound) {
    readNunchukAxes(xaxis, yaxis);
    if (xaxis > JOYSTICK_EXTREME_HIGH && yaxis > JOYSTICK_CENTER_MIN &&
        yaxis < JOYSTICK_CENTER_MAX) { // joystick right postition
      tempDriveMode = 1;
      modeFound = true;
    } else if (yaxis < JOYSTICK_EXTREME_LOW && xaxis > JOYSTICK_CENTER_MIN &&
               xaxis < JOYSTICK_CENTER_MAX) { // joystick down postition
      tempDriveMode = 2;
      modeFound = true;
    } else if (xaxis < JOYSTICK_EXTREME_LOW && yaxis > JOYSTICK_CENTER_MIN &&
               yaxis < JOYSTICK_CENTER_MAX) { // joystick left postition
      tempDriveMode = 3;
      modeFound = true;
    } else if (yaxis > JOYSTICK_EXTREME_HIGH && xaxis > JOYSTICK_CENTER_MIN &&
               xaxis < JOYSTICK_CENTER_MAX) { // joystick up postition
      tempDriveMode = 4;
      modeFound = true;
    }
