...
- `Bobby_Car_Controller/src/main.cpp` is the main controller entry point (setup, loop, and drive mode state flow).
- `Bobby_Car_Controller/include/config.h` holds hardware pins, serial baud rates, timing, calibration limits, and driving mode configurations.
- `Bobby_Car_Controller/include/hoverboard.h` & `src/hoverboard.cpp` manage UART communication and packet framing for the hoverboard motor controller.
- `Bobby_Car_Controller/include/nunchuk_input.h` & `src/nunchuk_input.cpp` provide high-level Nunchuk acquisition, fallback, and throttle/brake mapping.
- `Bobby_Car_Controller/include/nunchuk.h` holds the local low-level Nunchuk I2C driver library.
...
- Keep hardware pin, baud, calibration, and protocol constants centralized in `include/config.h`.
